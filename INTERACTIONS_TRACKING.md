# Comprehensive Interactions Tracking
...
- **Implemented:** Fully reactive, state-driven, or routes correctly.
...
- **Navigation Links (Dashboard, Schedule, Classes, etc.)**: Route to respective pages without full page reload (SPA routing).
...
- **Profile Dropdown Toggle**: Click to open/close the user menu.
- **Profile Menu - Account Settings**: Route to settings page or open modal.
- **Profile Menu - Help & Support**: Route to support page or open modal.
- **Profile Menu - Sign Out**: Clear auth state and route to `/login`.
- **Notifications Bell**: Click to open/close the notifications panel. Shows unread badge.
- **Notifications Panel - Mark all as read**: Clear unread state.
...
- **Search Input**: Instantly filters the table by teacher name or subject as you type.
- **"Core classes" Filter Dropdown**: Opens menu to filter by All, Core, or Enrichment.
- **"Select All" Button**: Toggles the selection state of all visible rows.
- **"Create Teacher" Button**: Opens the Create Teacher modal overlay.
- **Create Teacher Modal - Inputs**: Focus states for Name, Subjects, Email.
- **Create Teacher Modal - Cancel/X**: Closes the modal.
- **Create Teacher Modal - Save**: Validates and adds teacher to the list (currently closes modal).
- **Table Row Checkboxes**: Toggles individual row selection state.
- **Table Row Ellipses (...)**: Opens a row-specific action dropdown.
- **Row Action - Edit Teacher**: Opens Edit modal populated with teacher data.
- **Row Action - View Schedule**: Routes to teacher's schedule view.
- **Row Action - Message**: Opens messaging modal.
- **Row Action - Remove**: Opens confirmation dialog to delete teacher.
- **Pagination (Arrows & Numbers)**: Dynamically changes the visible table data.
...
- **Search Input**: Instantly filters the table by student or parent name.
- **"Select All" Button**: Toggles the selection state of all visible rows.
- **Table Row Checkboxes**: Toggles individual row selection state.
- **Student Name Link**: Routes to the specific Student Profile (`/students/[id]`).
- **Top Stat Cards (Total, Scheduled, etc.)**: Click to filter the table by that status.
...
- **"Start your journey" (Login) Button**: Validates inputs, then routes to `/dashboard`.
