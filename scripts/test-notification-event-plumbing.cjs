...
assert.match(serverWrites, /title: input\.rows\.length === 1 \? "New class request submitted"/);
assert.match(serverWrites, /href: "\/dashboard\/classes\/requests"/);
assert.match(serverWrites, /title: input\.rows\.length === 1 \? "Class request submitted"/);
