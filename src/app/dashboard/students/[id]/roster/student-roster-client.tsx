...
export default function StudentRosterClient({
  ...
  studentId: string;
  initialStudents: StudentRosterRow[];
  initialSource: DataSource;
}) {
  const contextStudentId = studentId;
  const cacheKey = `/api/data/students/${encodeURIComponent(contextStudentId)}/roster`;

  const [students, setStudents] = useState<StudentRosterRow[]>(() => initialStudents);
  const [source, setSource] = useState<DataSource>(initialSource);
  const [isLoading, setIsLoading] = useState(initialStudents.length === 0 && initialSource !== "remote");
  ...
  useEffect(() => {
    let cancelled = false;
    mutateDashboardData(cacheKey, () => ({ students: initialStudents, source: initialSource }));
    async function loadRoster() {
      if (initialStudents.length === 0) setIsLoading(true);
      try {
  ...
  }, [cacheKey, initialSource, initialStudents]);
  ...
  const dataHint =
    source === "fallback"
      ? "Showing a starter roster while student records finish loading."
      : source === "unavailable"
        ? "Student roster records are temporarily unavailable."
        : "";
  ...
  const exportSelectedSpreadsheet = async () => {
    ...
    downloadCsv(
      `student-roster-selected-${contextStudentId || "export"}.csv`,
      ["Student", "Parent", "Age", "Status"],
      selected.map((s) => [s.name, s.parent, s.age, s.status]),
    );
